...
%
%   Modified by Michael Tikhonov to remove numbering of array elements
%
...
gap_c=struct('l',0,'r',0,'t',0,'b',0); % gaps around cell elements
...
vertstruct=true; % align struct vertically or horizontally
% ------------------
...
elseif prod(siz)>1 % non-(char/num/logical)-arrays (usually cell or struct arrays)
    ...
    for n=1:N
        for m=1:M
            ...
            titles{n,m}={sprintf(formatstr, sub2ind(siz, [n, subs{:}]))};
